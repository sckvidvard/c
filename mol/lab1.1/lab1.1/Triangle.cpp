...
void Triangle::decreaseSide(double percent) {
    double new_side_a = side_a * (1 - percent / 100);
    double new_side_b = side_b * (1 - percent / 100);

    // Переопределяем гипотенузу только если это не приведет к её уменьшению до 0
    double new_hypotenuse = sqrt(new_side_a * new_side_a + new_side_b * new_side_b);

    if (new_hypotenuse <= 0) {
        cout << "Ошибка: уменьшение стороны приведет к гипотенузе, равной или меньше нуля." << endl;
        return;
    }
